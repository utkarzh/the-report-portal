...
- **`.docx` export** is a sibling builder ([interview-letter-docx.ts](src/lib/interview-letter-docx.ts)) reusing `docx-template.ts`'s shared page geometry/palette constants, like meeting-prep-docx.ts — but renders the letter's paragraphs as plain text blocks rather than running the full markdown lexer/renderer, since this content is short structured prose, never headings/tables/lists.
