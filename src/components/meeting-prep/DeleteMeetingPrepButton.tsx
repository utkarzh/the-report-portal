...
export default function DeleteMeetingPrepButton({
  sessionId,
  title,
  redirectTo,
  variant = 'button',
  ...
  const label = title || 'this meeting preparation'
  ...
  async function handleDelete() {
    setLoading(true)
    setError(null)
    const res = await fetch(`/api/meeting-prep/${sessionId}`, { method: 'DELETE' })
    setLoading(false)
    if (!res.ok) {
  ...
  return (
    <>
      {variant === 'icon' ? (
        <button
          onClick={openModal}
          aria-label="Delete meeting preparation"
      ...
      <Modal
        ...
        title={`Delete "${label}"?`}
        description="This permanently deletes this meeting preparation and its output. This can't be undone."
        confirmLabel="Delete"
