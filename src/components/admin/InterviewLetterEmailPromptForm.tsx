...
export default function InterviewLetterEmailPromptForm({ company, label, initialPrompt }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/interview-letters/email-prompt/${company}`, {
      method: 'PATCH',
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-sm text-emerald-700">
          Prompt saved. New {label} emails will be drafted using this prompt.
        </div>
      )}
