...
export default function TranscriptPromptForm({ initialPrompt }: { initialPrompt: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/transcript-prompt', {
      method: 'PATCH',
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-sm text-emerald-700">
          Refining prompt saved. All future transcript refinements will use this prompt.
        </div>
      )}
      ...
      <div className="bg-white border border-[#e5e3df]">
        <div className="p-5 sm:p-6">
          <Textarea
            ...
            placeholder="Define the tone, structure and editorial style the AI should follow when refining transcripts..."
            value={promptText}
