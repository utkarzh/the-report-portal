...
import type { MeetingPrepPromptKey } from '@/types'
...
export default function MeetingPrepPromptForm({ promptKey, label, initialPrompt }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/meeting-prep/prompts/${promptKey}`, {
      method: 'PATCH',
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-sm text-emerald-700">
          Prompt saved. All future {label.toLowerCase()} stages will use this prompt.
        </div>
      )}
