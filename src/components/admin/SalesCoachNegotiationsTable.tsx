...
import { Search, X, Flag, HelpCircle } from 'lucide-react'
...
// happens instantly in the browser — same reasoning as the module's other
// admin tables (e.g. CategoriesList).
export default function SalesCoachNegotiationsTable({ rows, flaggedOnly }: { rows: NegotiationRow[]; flaggedOnly: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.company, r.submitted_by_name, r.country, r.media_publication]
        .filter(Boolean)
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-[#e5e3df] bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((r) => {
                const uvCount = r.uv_criteria?.length || 0
                return (
