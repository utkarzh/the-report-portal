...
export default function DeleteInterviewLetterButton({
  projectId,
  title,
  redirectTo,
  variant = 'button',
  ...
  const label = title || 'this project'
  ...
  async function handleDelete() {
    setLoading(true)
    setError(null)
    const res = await fetch(`/api/interview-letters/${projectId}`, { method: 'DELETE' })
    setLoading(false)
    if (!res.ok) {
  ...
  return (
    <>
      {variant === 'icon' ? (
        <button
          onClick={openModal}
          aria-label="Delete interview letter project"
      ...
      <Modal
        ...
        title={`Delete "${label}"?`}
        description="This permanently deletes this project, its letter/email drafts, and any personalized outputs. This can't be undone."
        confirmLabel="Delete"
