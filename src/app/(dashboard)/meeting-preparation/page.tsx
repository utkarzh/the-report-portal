...
export default async function MeetingPreparationPage({ searchParams }: { searchParams: { page?: string; search?: string } }) {
  ...
  if (search) {
    query = query.or(orIlikeFilter(['interviewee_name', 'company_org', 'publication'], search))
  }
  ...
  return (
    <div className="p-8">
        ...
        <div className="mb-6 rounded-xl border border-[#c8973f]/25 bg-[#fbf7ed] p-4 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/admin/meeting-prep" className="group inline-flex items-center gap-2 rounded-lg border border-[#c8973f]/25 bg-white px-3.5 py-2 text-sm font-medium text-gray-700 shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#c8973f]/50 hover:text-[#a07530] hover:shadow">
      ...
      <div className="mb-6">
        <SearchInput basePath="/meeting-preparation" initialValue={search} placeholder="Search by interviewee, company, publication..." />
      </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-[#e5e3df] bg-white p-8 text-sm text-gray-500 shadow-sm flex items-start gap-3">
          ...
          {search ? (
            <span>
              No meeting preparations match &ldquo;<span className="font-medium text-gray-700">{search}</span>&rdquo;.{' '}
              <Link href="/meeting-preparation" className="text-gray-700 underline hover:text-black">Clear search</Link>
            </span>
          ) : (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {items.map((s, i) => (
              <EntityCard
                ...
                title={s.interviewee_name}
                subtitle={[s.company_org, s.publication].filter(Boolean).join(' · ')}
                date={new Date(s.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                creatorName={s.creatorName}
                badge={
                  <StatusPill
                    label={STAGE_LABELS[s.stage] || s.stage}
                    tone={s.stage === 'complete' ? 'emerald' : s.stage === 'failed' ? 'red' : 'amber'}
                    pulse={s.stage !== 'complete' && s.stage !== 'failed'}
                  />
                }
                footerLabel="View meeting prep"
                deleteSlot={profile.role === 'admin' ? <DeleteMeetingPrepButton sessionId={s.id} title={s.interviewee_name} variant="icon" /> : undefined}
              />
            ))}
          </div>
